...
func transfer(_ transferType: String) {
    switch transferType {
    case "withdraw": if accountType == "debit" {
        debitWithdraw(_: transferAmount)
    }else {
        withdraw(_: transferAmount)
    }
    case "deposit": if accountType == "credit" {
        creditDeposit(_: transferAmount)
    } else {
        deposit(_: transferAmount)
    }
    default: break
    }
}
